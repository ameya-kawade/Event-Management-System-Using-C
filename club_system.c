// club_system.c
// Updated single-file implementation with "Most Active Members" feature
// Architecture A: top-level HashMap (bucket -> AVL tree) for Departments & Events
...
static void persist_write_feedback_avl(AVL_Node* root, int event_id) { if (!root || !persist_feedback_fp) return; persist_write_feedback_avl(root->left, event_id); Feedback* f = (Feedback*)root->data; PersistFeedback pf; pf.event_id = event_id; pf.fb = *f; fwrite(&pf, sizeof(PersistFeedback), 1, persist_feedback_fp); persist_write_feedback_avl(root->right, event_id); }
...
static void persist_save_departments(IT_Club_System* sys) {
    ...
    for (int i = 0; i < sys->department_map->size; ++i) {
        AVL_Node* root = (AVL_Node*)sys->department_map->buckets[i]; if (!root) continue;
        void write_dept(void* data) {
            Department* d = (Department*)data; PersistDepartment pd; pd.dept_id = d->dept_id; strncpy(pd.name, d->name, sizeof(pd.name)-1); pd.name[sizeof(pd.name)-1] = '\0'; pd.member_count = d->member_count; fwrite(&pd, sizeof(PersistDepartment), 1, persist_fp_global);
            for (int b = 0; b < d->member_map->size; ++b) { AVLChain* cur = (AVLChain*)d->member_map->buckets[b]; while (cur) { if (cur->owner_id == d->dept_id) avl_inorder(cur->tree, persist_write_member_cb); cur = cur->next; } }
        }
...
static void persist_save_events_and_feedback(IT_Club_System* sys) {
    ...
    for (int i = 0; i < sys->event_map->size; ++i) { AVL_Node* root = (AVL_Node*)sys->event_map->buckets[i]; if (!root) continue; void write_event(void* data) { Event* e = (Event*)data; PersistEvent pe; pe.event_id = e->event_id; strncpy(pe.name, e->name, sizeof(pe.name)-1); pe.name[sizeof(pe.name)-1] = '\0'; strncpy(pe.venue, e->venue, sizeof(pe.venue)-1); pe.venue[sizeof(pe.venue)-1] = '\0'; strncpy(pe.date, e->date, sizeof(pe.date)-1); pe.date[sizeof(pe.date)-1] = '\0'; pe.budget = e->budget; pe.expenses = e->expenses; pe.profit_loss = e->profit_loss; strncpy(pe.description, e->description, sizeof(pe.description)-1); pe.description[sizeof(pe.description)-1] = '\0'; pe.member_count = e->member_count; pe.feedback_count = e->feedback_count; fwrite(&pe, sizeof(PersistEvent), 1, persist_fp_global); for (int b = 0; b < e->member_map->size; ++b) { AVLChain* cur = (AVLChain*)e->member_map->buckets[b]; while (cur) { if (cur->owner_id == e->event_id) avl_inorder(cur->tree, persist_write_member_cb); cur = cur->next; } } } avl_inorder(root, write_event); }
    persist_fp_global = NULL; fclose(efp);

    FILE* ffp = fopen("feedback.dat", "wb"); if (!ffp) { printf("[ERR] cannot open feedback.dat\n"); return; }
    int total_feedback = 0; for (int i = 0; i < sys->event_map->size; ++i) { AVL_Node* root = (AVL_Node*)sys->event_map->buckets[i]; if (!root) continue; void count_fb(void* data) { Event* e = (Event*)data; total_feedback += e->feedback_count; } avl_inorder(root, count_fb); }
    fwrite(&total_feedback, sizeof(int), 1, ffp);
    persist_feedback_fp = ffp;
    for (int i = 0; i < sys->event_map->size; ++i) { AVL_Node* root = (AVL_Node*)sys->event_map->buckets[i]; if (!root) continue; void write_fbs(void* data) { Event* e = (Event*)data; for (int b = 0; b < e->feedback_map->size; ++b) { AVLChain* cur = (AVLChain*)e->feedback_map->buckets[b]; while (cur) { if (cur->owner_id == e->event_id) persist_write_feedback_avl(cur->tree, e->event_id); cur = cur->next; } } } avl_inorder(root, write_fbs); }
...
static void persist_load_departments(IT_Club_System* sys) {
    FILE* fp = fopen("departments.dat", "rb"); if (!fp) return; int cnt = 0; if (fread(&cnt, sizeof(int), 1, fp) != 1) { fclose(fp); return; }
    for (int i = 0; i < cnt; ++i) { PersistDepartment pd; if (fread(&pd, sizeof(PersistDepartment), 1, fp) != 1) break; Department* d = department_create(pd.dept_id, pd.name); for (int j = 0; j < pd.member_count; ++j) { Member* m = (Member*)malloc(sizeof(Member)); if (fread(m, sizeof(Member), 1, fp) != 1) { free(m); break; } hmchain_insert_record(d->member_map, m->member_id, m, d->dept_id); d->member_count++; } hms_insert_top(sys->department_map, d->dept_id, d); }
    fclose(fp);
}

static void persist_load_events(IT_Club_System* sys) {
    FILE* fp = fopen("events.dat", "rb"); if (!fp) return; int cnt = 0; if (fread(&cnt, sizeof(int), 1, fp) != 1) { fclose(fp); return; }
    for (int i = 0; i < cnt; ++i) { PersistEvent pe; if (fread(&pe, sizeof(PersistEvent), 1, fp) != 1) break; Event* e = event_create(pe.event_id, pe.name, pe.venue, pe.date, pe.budget, pe.description); e->expenses = pe.expenses; e->profit_loss = pe.profit_loss; for (int j = 0; j < pe.member_count; ++j) { Member* m = (Member*)malloc(sizeof(Member)); if (fread(m, sizeof(Member), 1, fp) != 1) { free(m); break; } hmchain_insert_record(e->member_map, m->member_id, m, e->event_id); e->member_count++; } hms_insert_top(sys->event_map, e->event_id, e); }
    fclose(fp);
}

static void persist_load_feedbacks(IT_Club_System* sys) {
    FILE* fp = fopen("feedback.dat", "rb"); if (!fp) return; int total = 0; if (fread(&total, sizeof(int), 1, fp) != 1) { fclose(fp); return; }
    for (int i = 0; i < total; ++i) { PersistFeedback pf; if (fread(&pf, sizeof(PersistFeedback), 1, fp) != 1) break; Event* e = (Event*)hms_get_top(sys->event_map, pf.event_id); if (!e) continue; Feedback* f = (Feedback*)malloc(sizeof(Feedback)); *f = pf.fb; hmchain_insert_record(e->feedback_map, f->feedback_id, f, e->event_id); e->feedback_count++; }
    fclose(fp);
}
...
/* =======================
   Crypto (XOR)
   ======================= */
static int crypto_generate_key() { srand((unsigned)time(NULL) ^ (unsigned)getpid()); return rand() % 255 + 1; }
static void xor_file(const char* in, const char* out, int key) { FILE* fi = fopen(in, "rb"); if (!fi) { printf("[WARN] %s not found\n", in); return; } FILE* fo = fopen(out, "wb"); if (!fo) { fclose(fi); printf("[ERR] cannot open %s\n", out); return; } int c; while ((c = fgetc(fi)) != EOF) fputc(c ^ key, fo); fclose(fi); fclose(fo); printf("[XOR] %s -> %s\n", in, out); }
static void encrypt_all(int key) { xor_file("departments.dat", "departments.enc", key); xor_file("events.dat", "events.enc", key); xor_file("feedback.dat", "feedback.enc", key); }
static void decrypt_all(int key) { xor_file("departments.enc", "departments.dat", key); xor_file("events.enc", "events.dat", key); xor_file("feedback.enc", "feedback.dat", key); }

/* =======================
   Auto-save thread
   ======================= */
static void* autosave_thread(void* arg) { IT_Club_System* sys = (IT_Club_System*)arg; while (1) { sleep(300); pthread_mutex_lock(&sys->data_lock); persist_save_all(sys); pthread_mutex_unlock(&sys->data_lock); } return NULL; }

/* =======================
   Simple recursive data-only heap for Events (used earlier)
   We will add a separate small heap for ParticipationRecord below
   ======================= */
typedef struct Heap {
    Event** data;
    int size;
...
static void heapify_down_recursive(Heap* h, int idx) { int left = 2*idx + 1; int right = 2*idx + 2; if (left >= h->size) return; int largest = idx; if (heap_score(h->data[left]) > heap_score(h->data[largest])) largest = left; if (right < h->size && heap_score(h->data[right]) > heap_score(h->data[largest])) largest = right; if (largest != idx) { heap_swap(h, idx, largest); heapify_down_recursive(h, largest); } }
...
static void pheap_down(PHeap* h, int idx) { int l = 2*idx + 1; int r = 2*idx + 2; if (l >= h->size) return; int largest = idx; if (pheap_score(h->data[l]) > pheap_score(h->data[largest])) largest = l; if (r < h->size && pheap_score(h->data[r]) > pheap_score(h->data[largest])) largest = r; if (largest != idx) { pheap_swap(h, idx, largest); pheap_down(h, largest); } }
...
static void reporting_top_k(IT_Club_System* sys, int k) { Heap* h = heap_create(1024); reporting_heap = h; for (int i = 0; i < sys->event_map->size; ++i) avl_inorder((AVL_Node*)sys->event_map->buckets[i], reporting_heap_cb); reporting_heap = NULL; printf("\n=== Top %d Events by Profit/Loss ===\n", k); for (int i = 0; i < k && h->size > 0; ++i) { Event* e = heap_extract_max(h); printf("%d. %s (ID:%d) — Profit: %.2f\n", i+1, e->name, e->event_id, e->profit_loss); } free(h->data); free(h); }

/* =======================
   Most Active Member Feature
   ======================= */

// Helper to insert ParticipationRecord into participation map from member pointer
...
// callback used when seeding participation_map from department members
static HashMap_Chain* g_part_map_tmp = NULL;
static int g_part_owner_tmp = 0;
static void seed_participation_cb(void* data) {
    Member* m = (Member*)data; if (!m || !g_part_map_tmp) return;
    participation_insert_seed(m, g_part_map_tmp, g_part_owner_tmp);
}

// callback when counting participation: for each member in event, if member belongs to dept, increment count
...
static void count_participation_member_cb(void* data) {
    ...
    // match by department name (members stored with department_name when added to department)
...
// helper to traverse participation map and push into PHeap
static PHeap* g_ph_for_push = NULL;
static void push_participation_cb(void* data) {
    ParticipationRecord* r = (ParticipationRecord*)data; if (!r || !g_ph_for_push) return;
...
static void display_most_active_members(IT_Club_System* sys) {
    ...
    // 1) Create participation map (mode=1) keyed by member_id, owner_id = dept_id
    HashMap_Chain* part_map = create_HashMap_Chain(64, 1);

    // 2) Seed participation map with all members of department with count=0
    g_part_map_tmp = part_map; g_part_owner_tmp = d->dept_id;
    // traverse department's member_map and seed
    ...
    // 3) Iterate all events; for each event traverse its member_map and if member belongs to dept, increment count in part_map
    ...
    for (int i = 0; i < sys->event_map->size; ++i) {
        ...
        void per_event_cb(void* edata) {
            Event* e = (Event*)edata;
            if (!e) return;
            // traverse members of this event
    ...
    // 4) Push all participation records into PHeap
    PHeap* ph = pheap_create(256);
    g_ph_for_push = ph;
    // iterate part_map buckets
    ...
    // NOTE: we do not free ParticipationRecord or heap memory here to keep code simple; in production free them.
}
...
static void interactive_create_department(IT_Club_System* sys) {
    ...
    char name[100]; printf("Enter Department Name: "); input_read_line(name, sizeof(name)); Department* d = department_create(id, name); hms_insert_top(sys->department_map, id, d); printf("Department '%s' added (ID:%d)\n", name, id);
}
...
static void interactive_add_member_department(IT_Club_System* sys) {
    ...
    Member* m = (Member*)malloc(sizeof(Member)); *m = tmp; hmchain_insert_record(d->member_map, m->member_id, m, d->dept_id); d->member_count++; printf("Member '%s' (ID:%d) added to Dept '%s'\n", m->name, m->member_id, d->name);
}
...
static void interactive_add_member_event(IT_Club_System* sys) {
    ...
    Member* m = (Member*)malloc(sizeof(Member)); *m = tmp; hmchain_insert_record(e->member_map, m->member_id, m, e->event_id); e->member_count++; printf("Member '%s' (ID:%d) added to Event '%s'\n", m->name, m->member_id, e->name);
}

static void interactive_remove_member_event(IT_Club_System* sys) {
    printf("Enter Event ID: "); int eid; if (!input_read_int(&eid)) return; Event* e = (Event*)hms_get_top(sys->event_map, eid); if (!e) { printf("Event not found.\n"); return; } printf("Enter Member ID to remove: "); int mid; if (!input_read_int(&mid)) return; hmchain_delete_record(e->member_map, mid, e->event_id); if (e->member_count > 0) e->member_count--; printf("Attempted removal of member %d from event %d\n", mid, eid);
}

static void interactive_add_feedback_event(IT_Club_System* sys) {
    printf("Enter Event ID: "); int eid; if (!input_read_int(&eid)) return; Event* e = (Event*)hms_get_top(sys->event_map, eid); if (!e) { printf("Event not found.\n"); return; } Feedback tmp; printf("Enter Feedback ID: "); if (!input_read_int(&tmp.feedback_id)) return; if (hmchain_get_record(e->feedback_map, tmp.feedback_id, e->event_id)) { printf("Feedback ID exists.\n"); return; } printf("Enter Member ID (author): "); input_read_int(&tmp.member_id); printf("Enter Rating (1-5): "); input_read_int(&tmp.rating); printf("Enter Comment: "); input_read_line(tmp.comment, sizeof(tmp.comment)); Feedback* f = (Feedback*)malloc(sizeof(Feedback)); *f = tmp; hmchain_insert_record(e->feedback_map, f->feedback_id, f, e->event_id); e->feedback_count++; printf("Feedback %d added to event %s\n", f->feedback_id, e->name);
}

static void interactive_edit_event_financials(IT_Club_System* sys) {
    printf("Enter Event ID to edit: "); int eid; if (!input_read_int(&eid)) return; Event* e = (Event*)hms_get_top(sys->event_map, eid); if (!e) { printf("Event not found.\n"); return; } printf("Current Budget: %.2f | Expenses: %.2f | Profit/Loss: %.2f\n", e->budget, e->expenses, e->profit_loss); double nb, ne; printf("Enter new Budget (press Enter to keep): "); if (!input_read_double(&nb)) nb = e->budget; printf("Enter new Expenses (press Enter to keep): "); if (!input_read_double(&ne)) ne = e->expenses; e->budget = nb; e->expenses = ne; e->profit_loss = nb - ne; printf("Updated: Profit/Loss = %.2f\n", e->profit_loss);
}
...
int main() {
    ...
    pthread_t tid; if (pthread_create(&tid, NULL, autosave_thread, sys) == 0) pthread_detach(tid); else printf("[WARN] autosave thread failed\n");

    while (1) {
        display_menu(); int choice; if (!input_read_int(&choice)) { printf("Invalid input.\n"); continue; }
